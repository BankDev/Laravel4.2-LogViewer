...
<html lang="{{ Config::get('app.locale') }}">
    ...
    <head>
        ...
        {{ HTML::style('packages/bankdev/logviewer/css/style.css') }}
        
        <link rel="shortcut icon" href="{{ URL::to('packages/bankdev/logviewer/ico/favicon.png') }}">
        <link rel="apple-touch-icon-precomposed" sizes="144x144" href="{{ URL::to('packages/bankdev/logviewer/ico/apple-touch-icon-144-precomposed.png') }}">
        <link rel="apple-touch-icon-precomposed" sizes="114x114" href="{{ URL::to('packages/bankdev/logviewer/ico/apple-touch-icon-114-precomposed.png') }}">
        <link rel="apple-touch-icon-precomposed" sizes="72x72" href="{{ URL::to('packages/bankdev/logviewer/ico/apple-touch-icon-72-precomposed.png') }}">
        <link rel="apple-touch-icon-precomposed" href="{{ URL::to('packages/bankdev/logviewer/ico/apple-touch-icon-57-precomposed.png') }}">
    ...
    <body>
                            ...
                            <ul class="nav nav-list">
                                @if ($logs)
                                    @foreach ($logs as $type => $files)
                                        @if ( ! empty($files['logs']) && (strtolower($files['sapi']) == strtolower($sapi_plain)) )
                                        ...
                                        @foreach ($log as $l)
                                            @if (strlen($l['stack']) > 1)
                                                <div class="alert alert-block alert-{{ $l['level'] }}">
                                                    ...
                                                    <span class="stack-header stack-header-wrap ">{{ $l['header'] }}</span>
        ...
        <script>window.jQuery || document.write('<script src="{{ URL::to("packages/bankdev/logviewer/js/jquery-1.10.2.min.js") }}"><\/script>')</script>
        {{ HTML::script('//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.1/js/bootstrap.min.js') }}
        {{ HTML::script('packages/bankdev/logviewer/js/script.js') }}
